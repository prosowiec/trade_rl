...
class RainbowDQN(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.adv2 = NoisyLinear(8 +1, output_dim)

        self.val1 = NoisyLinear(32, 8)
        self.val2 = NoisyLinear(8+1, 1)

    def forward(self, x, portfolio=[0]):
        ...
        if portfolio is None:
            ...
            portfolio = torch.zeros(batch_size, device=x.device)
        
        # Ensure portfolio is shape [batch]
        if portfolio.dim() == 0:  # scalar
            portfolio = portfolio.unsqueeze(0)
        
        # Flatten to [batch] if it has extra dimensions
        portfolio = portfolio.view(-1)
        
        # Expand to [batch, 1] for concat
        portfolio = portfolio.unsqueeze(1)

        adv = self.relu(self.adv1(lstm_out)) # [B, 8]
        #print("adv shape before concat:", adv.shape, portfolio.shape)
        adv = torch.cat([adv, portfolio], dim=1)
        #print("adv shape:", adv.shape)  
        adv = self.adv2(adv)

        val = self.relu(self.val1(lstm_out))
        val = torch.cat([val, portfolio], dim=1)
        val = self.val2(val)
...
class RainbowAgent:
    ...
    def compute_td_error(self, states,portfolio, actions, rewards, next_states, dones):
        with torch.no_grad():
            next_q = self.model(next_states, portfolio)
            next_actions = torch.argmax(next_q, dim=1)
            next_q_target = self.target_model(next_states,portfolio)
            max_next_q = next_q_target.gather(1, next_actions.unsqueeze(1)).squeeze(1)
            target_q = rewards + self.gamma * max_next_q * (1 - dones)
        current_q = self.model(states, portfolio).gather(1, actions.unsqueeze(1)).squeeze(1)
    ...
    def train(self):
        ...
        states,portfolio, actions, rewards, next_states, dones = batch
        states = torch.FloatTensor(np.array(states)).to(self.device)
        portfolio = torch.FloatTensor(np.array(portfolio)).to(self.device)
        ...
        td_errors = self.compute_td_error(states,portfolio, actions, rewards, next_states, dones)
    ...
    def get_action(self, state, portfolio=[0]):
        state_v = torch.FloatTensor(state).unsqueeze(0).to(self.device)
        with torch.no_grad():
            q_values = self.model(state_v, portfolio=portfolio)
